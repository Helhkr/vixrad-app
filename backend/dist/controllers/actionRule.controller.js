 ...
 * }
 */
export const createActionRule = async (req, res) => {
 ...
 */
export const deleteActionRule = async (req, res) => {
    // TODO: Adicionar verificação de administrador (req.user.isAdmin)
    try {
        const { id } = req.params; // Use 'id' from params as primary key
 ...
 */
export const getActionRuleByTemplateAndSourceActionId = async (req, res) => {
    try {
        ...
        if (!actionRule) {
            return res.status(404).json({ message: 'Regra de ação não encontrada.' });
        }
        res.status(200).json(actionRule);
    }
    catch (error) {
