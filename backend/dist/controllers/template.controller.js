                    ...
                    create: sections.map((section) => ({
                        ...
                        subsections: {
                            create: section.subsections?.map((subsection) => ({
                                ...
                                dynamicOptionSets: {
                                    create: subsection.dynamicOptionSets?.map((dynamicOptionSet) => ({
                                        title: dynamicOptionSet.title,
                                        display_order: dynamicOptionSet.display_order,
                                        elements: {
                                            create: dynamicOptionSet.elements?.map((element) => ({
 ...
 */
export const addSectionToTemplate = async (req, res) => {
    // A verificação de admin está comentada para fins de teste
    // if (!req.user?.isAdmin) {
    //   return res.status(403).json({ message: 'Acesso negado.' });
    // }
 ...
 */
export const addSubsectionToSection = async (req, res) => {
    // if (!req.user?.isAdmin) {
    //   return res.status(403).json({ message: 'Acesso negado.' });
    // }
    try {
 ...
 */
export const addDynamicOptionSetToSubsection = async (req, res) => {
    // if (!req.user?.isAdmin) {
    //   return res.status(403).json({ message: 'Acesso negado.' });
    // }
    try {
 ...
 */
export const updateDynamicOptionSet = async (req, res) => {
 ...
 */
export const deleteDynamicOptionSet = async (req, res) => {
    // if (!req.user?.isAdmin) { return res.status(403).json({ message: 'Acesso negado.' }); }
    try {
        const { id } = req.params;
 ...
 */
export const addInteractiveElementToDynamicOptionSet = async (req, res) => {
    // if (!req.user?.isAdmin) {
    //   return res.status(403).json({ message: 'Acesso negado.' });
    // }
    try {
 ...
 */
export const updateInteractiveElement = async (req, res) => {
    // if (!req.user?.isAdmin) { return res.status(403).json({ message: 'Acesso negado.' }); }
    try {
 ...
 */
export const deleteInteractiveElement = async (req, res) => {
    // if (!req.user?.isAdmin) { return res.status(403).json({ message: 'Acesso negado.' }); }
    try {
        const { elementId } = req.params;
