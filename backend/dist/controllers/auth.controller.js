...
export const register = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ userId: newUser.id, isAdmin: newUser.is_admin }, secret, { expiresIn: '1h' });
        res.status(201).json({
...
export const login = async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({ userId: user.id, isAdmin: user.is_admin }, accessTokenSecret, { expiresIn: '15m' });
...
export const verifyEmail = async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({ userId: updatedUser.id, isAdmin: updatedUser.is_admin }, accessTokenSecret, { expiresIn: '15m' });
        const refreshToken = jwt.sign({ userId: updatedUser.id }, refreshTokenSecret, { expiresIn: '7d' });
        res.cookie('refreshToken', refreshToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            sameSite: 'strict',
...
export const refresh = (req, res) => {
    ...
    jwt.verify(refreshToken, refreshTokenSecret, (err, user) => {
        ...
        const accessToken = jwt.sign({ userId: user.userId, isAdmin: user.isAdmin, is_email_verified: user.is_email_verified }, accessTokenSecret, { expiresIn: '15m' });
